...
def reset_voice_text() -> str:
    ...
    return ("No problem — let's start fresh. May I have your name please?")
...
def build_voice_text(product) -> str:
    ...
    parts = [f"Here's more on {name}."]

    if key_feature:
        parts.append(f"Its key feature: {key_feature}.")
    elif desc:
    ...
    if pmin and pmax:
        try:
            parts.append(f"Premium ranges from {int(pmin):,} to {int(pmax):,} INR per month.")
    ...
    if min_age is not None and max_age is not None:
        try:
            parts.append(f"Eligible from age {int(min_age)} to {int(max_age)}.")
        except (TypeError, ValueError):
            pass

    # Smoker — explicit interpretation (Bug 14: smoker_eligible:true means BOTH allowed).
    if smoker_ok is True:
        parts.append("Available for both smokers and non-smokers.")
    elif smoker_ok is False:
        parts.append("Available for non-smokers only.")
    ...
    if sum_max:
        try:
            ...
            if sum_max_int >= 10_000_000:
                parts.append(f"Maximum sum assured is {sum_max_int / 10_000_000:.1f} crore INR.")
            elif sum_max_int >= 100_000:
                parts.append(f"Maximum sum assured is {sum_max_int / 100_000:.0f} lakh INR.")
        except (TypeError, ValueError):
            pass

    parts.append("Would you like to hear about another option, or shall we proceed with this one?")
    text = " ".join(parts)
    return text
