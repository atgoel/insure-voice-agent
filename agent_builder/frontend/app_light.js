/* =================================------------------
 * InsureVoice â€” App Logic Orchestrator
...
btnMute.onclick = () => {
    ...
    if (isMuted) {
        ...
        if (window.voiceEngine) {
            ...
            // rebuild on un-mute â€” the exact failure class Strategy 2 removes.
...
function startVoiceOrbCanvas() {
    ...
    function renderOrb() {
        ...
        // Clear canvas transparent (light theme — no dark fill)
...
// ----------------------------------------------------
// Issue 1 (2026-06-06) â€” progress narration during the ~80s recommendation wait.
// The /invoke POST blocks synchronously for the full pipeline (search â†’
// compliance â†’ rank â†’ recommend, 5 sequential hops). With no signal during the
...
// flight. TEXT ONLY â€” deliberately NO spoken audio (spoken narration would
...
// "Advisor: â€¦" prefix; the spoken line is a natural sentence.
const _ORB_NARRATION_STAGES = [
    ...
    { label: 'Advisor: Preparing your top matches...',      say: "Almost there â€” preparing your top matches." },
];
...
// no voice mismatch. Played via a plain <audio> element (no MediaSource needed â€”
...
function _speakProgressChain() {
    ...
    const playNext = () => {
        ...
        audio.onerror = () => {
            window.__voiceMicSuspended = false;
            if (_spokenProgressCancelled) return;
            // Clip failed to load â€” advance label silently
            _orbNarrationTimer = setTimeout(playNext, 5000);
        };
        audio.play().catch(() => {
            // Autoplay blocked or file missing â€” advance silently
            window.__voiceMicSuspended = false;
...
const renderAgentMarkdown = (raw) => {
    if (!raw) return '';
    let s = String(raw);
    // Bold: **text** â†’ <strong>text</strong> (also __text__)
    s = s.replace(/\*\*([^*\n]+?)\*\*/g, '<strong>$1</strong>');
    s = s.replace(/__([^_\n]+?)__/g, '<strong>$1</strong>');
    // Italic: *text* (not list bullets) â†’ <em>text</em>
    s = s.replace(/(^|[^*])\*([^*\n]+?)\*(?!\*)/g, '$1<em>$2</em>');
    // List items: lines starting with "* " or "- " â†’ "â€¢ "
...
// Pure client-side derivation â€” no LLM, no backend change (per L-001: keep
...
// (smoker/family/health are compliance filters, not in the score â€” eligibility
...
function buildWhyMatch(prod) {
    ...
    // Always give at least one confidence cue â€” passing compliance is itself a
    // profile-tied fact (eligible for your age / smoker status / health).
    if (reasons.length === 0) {
        reasons.push("Passed all eligibility checks for your profile");
    }
    return reasons.slice(0, 2);  // keep cards tight â€” top 2 reasons
}
...
window.displayRecommendedProducts = function(passed, rejected = []) {
    ...
    passed.forEach((prod, i) => {
        ...
        // Issue 3 â€” per-card "Why it fits you" reasons from score_breakdown.
