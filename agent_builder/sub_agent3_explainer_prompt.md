...
- **Premiums are indicative** — the figures from `premium_min_monthly` / `premium_max_monthly` are ranges. Always present them as "from ₹X to ₹Y per month" or "around ₹X per month", never as a fixed quote.
